...
const ProjectTemplate = ({ images, title, text: TextComponent, imgStyles = {}, videoStyles = {}, instagram = null, link = null, linkText = "Visit Project", centerVideo = false, pdfUrl = null }) => {
  ...
  return (
    <section className="flex flex-col items-center w-full p-4 md:w-4/5 md:p-8 md:flex-row md:h-full">
      <div className="order-1 md:order-2 flex justify-center md:items-start w-full md:w-1/2 px-3 md:px-5 space-y-2 md:space-y-4 mt-4 md:mt-0 mb-6 md:mb-0">
        <div className="max-w-lg">
          <h1 className="text-xl md:text-2xl mt-2 md:mt-4">{title}</h1>
            ...
            return (
              <React.Fragment key={index}>
                {img.src.endsWith(".mp4") ? (
                  ...
                  <div 
                    key={index + "projectimg"} 
                    className={`flex items-center justify-center p-2 md:p-4 cursor-pointer ${isThirdImageOfThree ? 'col-span-2' : ''}`}
                    onClick={() => setSelectedMedia({ src: img.src, type: 'image', alt: img.alt })}
                  >
                    <img
                      src={img.src}
                      alt={img.alt}
                      className={`rounded-2xl object-cover transition-transform hover:scale-105 ${isThirdImageOfThree ? 'w-1/2' : 'w-full'}`}
                      style={imgStyles}
                      draggable="false"
                    />
                  </div>
